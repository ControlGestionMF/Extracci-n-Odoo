...
from dateutil.relativedelta import relativedelta  # Para restar meses
...
def extraer_pedidos_venta(uid, models, batch_size=100, max_records=5000):
    ...
    # Obtener product_id únicos para obtener templates y SKU
    ...
    # Obtener IDs únicos de impuestos de todas las líneas
    tax_ids = set()
    for linea in lineas:
        if 'tax_id' in linea and linea['tax_id']:
            tax_ids.update(linea['tax_id'])  # tax_id es lista de IDs

    # Leer nombres de impuestos
    impuestos = []
    if tax_ids:
